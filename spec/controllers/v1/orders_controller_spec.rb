  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "create a new line item with order" do
        # @order = FactoryGirl.attributes_for(:v1_order)
        # @lineitem = FactoryGirl.attributes_for(:v1_item_line)
        expect {
          post :create, 
        ...
        expect(V1::ItemLine.count).to eq 1
  ...
  describe "PUT update" do
    describe "with valid params" do
      ...
      it "updates the requested v1_order" do
        order = FactoryGirl.create(:v1_order)
        put :update, {:id => order.id, :v1_order => {vat: 10}}, token: @user.api_key.token
        order.reload
      ...
      it "can only be edit in draft state" do
         put :update,  :id => @order.id, :v1_order => {item_lines_attributes: [:id => 2, :quantity => 4, :net_price => 80.1]}, token: @user.api_key.token
         @order.reload
         @item2.reload
